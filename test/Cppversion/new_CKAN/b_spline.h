...
void b_splines(const fixed_t x[1024*2]){
    ...
    for (fixed_i i = -spline_order; i <= grid_size + spline_order + 1; ++i) {
    ...
    for (fixed_i i = 0; i < get_dim(3,x_dim); ++i) {
        for (fixed_i j = 0; j < get_dim(2,x_dim); ++j) {
            for (fixed_i k = 0; k < get_dim(1,x_dim); ++k) {
                ...
                for(fixed_i l=0;l<len(grid_dim)-1;l++){
    ...
    for(fixed_i t=1;t<=spline_order;t++){
        for(fixed_i i=0;i<get_dim(3,bases_dim);i++){
            for(fixed_i j=0;j<get_dim(2,bases_dim);j++){//data
                fixed_t target=x[get_index(0,i,j,x_dim)];
                for(fixed_i k=0;k<get_dim(1,bases_dim);k++){//every knots
